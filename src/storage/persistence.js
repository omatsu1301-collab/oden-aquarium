...
function persistFreshState(nowMs) {
  const state = createInitialGameState(nowMs, createFreshSeed());
  writeRaw(LOCAL_STORAGE_KEY, JSON.stringify(state));
...
export function loadGameState(nowMs) {
  ...
  if (raw === null) {
    return persistFreshState(nowMs);
  }
  ...
  if (isV1SaveData(parsed)) {
    ...
    writeRaw(LOCAL_STORAGE_KEY, JSON.stringify(migrated));
...
export function saveGameState(state) {
  return writeRaw(LOCAL_STORAGE_KEY, JSON.stringify(state));
}

// 復元(バックアップ/インポート)専用: 検証済み状態をそのまま置き換える。
// 置換前に必ず現在の状態の退避に成功していることを呼び出し側が保証する。
export function overwriteGameState(state) {
  return writeRaw(LOCAL_STORAGE_KEY, JSON.stringify(state));
}

export function createFreshGameState(nowMs) {
  return persistFreshState(nowMs);
}
